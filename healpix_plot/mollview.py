...
def _mollweide_inverse(x, y, central_lon_rad=0.0):
    ...
    lat_rad   = np.arcsin(np.clip(
        (2.0 * theta + np.sin(2.0 * theta)) * _INV_PI, -1.0, 1.0))
...
def _gnomonic_inverse(x, y, lon0_deg, lat0_deg):
    ...
    lon = lon0 + np.arctan2(x * sin_c,
                             safe_rho * cos_lat0 * cos_c - y * sin_lat0 * sin_c)
...
def _rasterise(hpx_map, depth, lon_grid, lat_grid, valid, nest, ellipsoid):
    ...
    if valid.any():
        ...
        ipix = fn(lon_grid.ravel()[flat], lat_grid.ravel()[flat],
...
def _build_cmap(cmap, bad_color):
    obj = (plt.get_cmap(cmap).copy() if isinstance(cmap, str)
...
def _build_norm(data_img, vmin, vmax, norm):
    ...
    return mcolors.Normalize(vmin=vmin if vmin is not None else lo,
...
def _draw_graticule_moll(ax, step_deg, central_lon_rad,
                          lw=0.5, color="white", ls="--", alpha=0.7, n_pts=360):
...
def _finalize_moll_axes(ax):
    ...
    ell_border = Ellipse(xy=(0, 0), width=2 * _2SQRT2, height=2 * _SQRT2,
                         facecolor="none", edgecolor="white", linewidth=0.8,
                         zorder=3, transform=ax.transData)
    ax.add_patch(ell_border)
    clip_ell = Ellipse(xy=(0, 0), width=2 * _2SQRT2, height=2 * _SQRT2,
...
def _draw_graticule_on_geoaxes(ax, step_deg, central_lon_rad,
                                 lw=0.5, color="white", ls="--", alpha=0.7,
                                 n_pts=360):
    """
    ...
    for lat in np.arange(-90.0, 90.0 + 1e-9, step):
        ...
        xp, yp = _mollweide_forward(lons_r, np.full_like(lons_r, lat_c), central_lon_rad)
        ax.plot(xp, yp,
                color=color,
                lw=lw * 1.6 if lat == 0.0 else lw,
                ls="-"      if lat == 0.0 else ls,
                alpha=alpha, zorder=2, transform=native)
    for lon in np.arange(-180.0, 180.0 + 1e-9, step):
        xp, yp = _mollweide_forward(np.full_like(lats_r, lon), lats_r, central_lon_rad)
        ax.plot(xp, yp, color=color, lw=lw, ls=ls, alpha=alpha,
                zorder=2, transform=native)
...
def mollview(
    ...
    * coastlines=True -> cartopy path (~500 ms).
    ...
    if not coastlines:
        ...
        data_img = _rasterise(hpx_map, depth, lon_grid, lat_grid, valid,
                              nest, ellipsoid)
    ...
    * coastlines=True  -> cartopy path (requires cartopy).
    ...
    if not coastlines:
        ...
        for v in np.arange(-half, half + 1e-9, step):
            ...
            lo_v, _  = _gnomonic_inverse(np.array([v]),   np.array([0.0]),
                                         lon_center, lat_center)
            _,  la_v = _gnomonic_inverse(np.array([0.0]), np.array([v]),
                                         lon_center, lat_center)
            ax.text(v,            -half * 1.02, f"{lo_v[0]:.1f}deg",
                    color="gray", fontsize=6, ha="center", va="top")
            ax.text(-half * 1.02, v,            f"{la_v[0]:.1f}deg",
                    color="gray", fontsize=6, ha="right",  va="center")
        ...
        ax.imshow(
            data_img,
            origin="lower",
            extent=[lon_center - half, lon_center + half,
                    lat_center - half, lat_center + half],
            transform=plate_cr,
            cmap=cmap_obj,
            norm=norm_obj,
